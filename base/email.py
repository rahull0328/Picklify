...
def send_account_activation_email(email, email_token):
    ...
    # Attach the image
    logo_path = os.path.join(settings.BASE_DIR, "public/static/img/main_logo.png")  # Adjust path as needed
    try:
        with open(logo_path, "rb") as img_file:
            logo = MIMEImage(img_file.read(), _subtype="webp")
            logo.add_header("Content-ID", "<logo>")  # Content-ID for embedding
            logo.add_header("Content-Disposition", "inline", filename="main_logo.png")
...
def send_order_invoice_email(email, invoice_path, context):
    ...
    # Create the email
    email_message = EmailMessage(subject, plain_message, email_from, [email])
